...
<s:action name="userList" namespace="/user"/>
<%
...
<div style="position: absolute;left: 35%;top: 15%;width: 53%;">
    ...
    <div class="panel panel-default">
        ...
        <table class="table">
            <table class="table">
                <script>
                    function edit(name, tp) {
                        $.ajax({
                            url: '/user/userTypeUpdate',
                            type: 'post',
                            data: {username: name, userType: tp},
                            dataType: 'text',
                            success: function (data) {
                                window.location.href = '/project/user.jsp';
                            }
                        });
                    }
                </script>
                ...
                <tr>
                    ...
                    <td><h5><span
                            class="label label-<%=new String[]{"danger", "warning", "primary", "info"}[u.getUserType()]%>"><%=new String[]{"管理员", "教师", "学生", "访客"}[u.getUserType()]%></span>
                    </h5></td>
                    <td>
                        <div class="btn-group btn-group-sm" role="group" aria-label="...">
                            <button type="button" class="btn btn-info" onclick="edit('<%=u.getUsername()%>', 3)">访客
                            </button>
                            <button type="button" class="btn btn-primary" onclick="edit('<%=u.getUsername()%>', 2)">学生
                            </button>
                            <button type="button" class="btn btn-warning" onclick="edit('<%=u.getUsername()%>', 1)">教师
                            </button>
                            <button type="button" class="btn btn-danger" onclick="edit('<%=u.getUsername()%>', 0)">管理员
                            </button>
                        </div>
                    </td>
                </tr>
    ...
    <%--<td><%=u.getUsername()%></td>--%>
    <%--<td><%=u.getRealName()%></td>--%>
    <%--<td><%=new String[]{"女", "男", "保密"}[u.getGender()]%></td>--%>
    <%--<td><%=new String[]{"管理员", "教师", "学生", "访客"}[u.getUserType()]%></td>--%>
    <%--</tr>--%>
    <%--<%--%>
    <%--}--%>
    <%--}--%>
